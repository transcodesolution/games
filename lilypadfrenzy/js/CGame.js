function CGame(oData, iLevel, iScore) {
    ...
    this._init = function(iLevel, iScore) {
        ...
        if (s_bMobile === false) {
            var oParent = this;
            s_oStage.addEventListener("stagemousemove", function(evt) {
                oParent._onMouseMove(evt.stageX, evt.stageY)
            });
        }
    ...
    this._initCurve = function() {
        ...
        // DRAW LEFT CURVE
        oPoint = {
            x: (_aCurveMapData[1][0] - _aCurveMapData[0][0]),
            y: (_aCurveMapData[1][1] - _aCurveMapData[0][1])
        };
        ...
        for (h = 1; h < _aCurveMapData.length - 1; h++) {
            ...
            oPoint = {
                x: (_aCurveMapData[h + 1][0] - _aCurveMapData[h][0]),
                y: (_aCurveMapData[h + 1][1] - _aCurveMapData[h][1])
            };
            oPoint = this._normalize(oPoint);
            oPoint = this._rot90CW(oPoint);
        ...
        // DRAW RIGHT CURVE
        oPoint = {
            x: (_aCurveMapData[_aCurveMapData.length - 1][0] - _aCurveMapData[_aCurveMapData.length - 2][0]),
        ...
        for (h = _aCurveMapData.length - 2; h > 1; h--) {
            oPoint = {
                x: (_aCurveMapData[h][0] - _aCurveMapData[h - 1][0]),
                y: (_aCurveMapData[h][1] - _aCurveMapData[h - 1][1])
            };
            oPoint = this._normalize(oPoint);
            oPoint = this._rot90CCW(oPoint);
    ...
    this._insertBall = function(oBall, iIndex, szLabel) {
        ...
        var oParent = this;
        createjs.Tween.get(oBall.getSprite()).to({
            x: iPosX,
            y: iPosY
        }, 200).call(function() {
