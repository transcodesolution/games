function CStartAnimation(oParentContainer, aWhitePos, aBlackPos, oAnimationContainer) {
    ...
    this._init = function(oParentContainer, aWhitePos, aBlackPos, oAnimationContainer) {
        ...
        for (var i = 0; i < aWhitePos.length; i++) {
            _aWhitePos[i] = {
                x: aWhitePos[i].x / s_oStage.scaleX,
                y: aWhitePos[i].y / s_oStage.scaleY
            };
            _aBlackPos[i] = {
                x: aBlackPos[i].x / s_oStage.scaleX,
                y: aBlackPos[i].y / s_oStage.scaleY
            };
        }
    ...
    this._openBox = function() {
        ...
        createjs.Tween.get(_oTopContainer).to({
            y: _oTopContainer.y - 160
        }, 1000).call(function() {
            _oParent._onBoxOpen();
        });
        createjs.Tween.get(_oBotContainer).to({
            y: _oBotContainer.y + 160
        }, 1000).call(function() {
            _oParent._onBoxOpen();
        });
    };
    ...
    this._onBoxOpen = function() {
        ...
        if (_iBoxOpenCounter === 2) {
            ...
            for (var i = 0; i < _iNumChips; i++) {
                ...
                createjs.Tween.get(_aBlackChips[i]).wait(iWaitTime).to({
                    x: _aBlackPos[i].x - PAWN_SIZE / 2 + 2,
                    y: _aBlackPos[i].y - PAWN_SIZE / 2 + 1
                ...
                createjs.Tween.get(_aWhiteChips[i]).wait(iWaitTime).to({
                    x: _aWhitePos[_iNumChips - i - 1].x - PAWN_SIZE / 2 + 1,
                    y: _aWhitePos[_iNumChips - i - 1].y - PAWN_SIZE / 2 + 1
                }, 500, createjs.Ease.cubicOut).
                call(function() {
                    playSound("click_cell", 1, false);
